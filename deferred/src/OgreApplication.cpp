...
void OgreApplication::InitLights()
{
	...
	msg << "[LIGHT] Num lights in OSM scene: " << lights.size() << std::endl;
	...
	{
		// Local vars
		const float SCALE_SCALAR = 1000.0f;
...
void OgreApplication::LoadScene(const std::string &file)
{
	...
	// Initialize scene lights for effects
	this->InitLights();
}
...
bool OgreApplication::keyPressed(const OIS::KeyEvent &keyEventRef)
{
	...
	if (this->_keyboard->isKeyDown(OIS::KC_1))
	{
		...
		{
			this->_log->logMessage("Deferred lighting enabled");
			this->_deferred_lighting_effect->Hide();
		}
		else
		{
			this->_log->logMessage("Deferred lighting disabled");
