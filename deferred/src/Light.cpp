...
const std::string tsdc::LightManager::POINT_LIGHT_MATERIAL = "tsdc/material/lights/point";
const std::string tsdc::LightManager::SPOT_LIGHT_MATERIAL = "tsdc/material/lights/spotlight";
...
#if 0
	std::stringstream msg;
	msg << "[OGRE] Accessing material for light: " << this->_light->getName();
	Ogre::LogManager::getSingleton().logMessage(msg.str());
#endif
	assert ( !this->_light_material.isNull() );
...
Light* LightManager::CreateLight(Ogre::Light *light, const Light::CtorParams *params)
{
	...
	new_light->LightMaterial( this->LightMaterial(light->getType()) );
	this->_lights.push_back(new_light);
#if 0
	Ogre::LogManager::getSingleton().logMessage("[LIGHT] Created deferred light: " + light->getName());
	Ogre::LogManager::getSingleton().logMessage("[LIGHT]\t=> Deferred light material: " + this->_point_light_material->getName());
#endif
	return new_light;
}
